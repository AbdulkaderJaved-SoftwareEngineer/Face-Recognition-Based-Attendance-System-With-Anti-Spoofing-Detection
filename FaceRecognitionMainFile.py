...
class StudentManagementApp:
    ...
    def capture_image(self):
        ...
        if not roll_no:
            messagebox.showerror("MHSSCOE : Error", "Roll number is required to save the Profile.")
            return
        elif not self.faceShown:
            messagebox.showerror("MHSSCOE : Face Not Detected", "Please make sure your face is properly visible and then try to capture the image ")
            return
        if ret:
            img_name = f"ImagesBasic/{roll_no}.png"
            cv2.imwrite(img_name, frame)
            messagebox.showinfo("MHSSCOE : Profile saved Successfully", f"Image saved as {img_name}")
