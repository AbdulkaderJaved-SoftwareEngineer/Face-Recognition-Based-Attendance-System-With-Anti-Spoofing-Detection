...
def detect(img, faceCascade, eyeCascade=None, noseCascade=None, mouthCascade=None):
    ...
    if len(coords)==4:
        ...
        #coords = draw_boundary(roi_img, eyeCascade, 1.1, 12, color['red'],'Eyes{}'.format(total_votes))
        ...
        # Detect nose
        nose = noseCascade.detectMultiScale(roi_img)
        if len(nose) >= 1:
            votes += 1
            print('Nose Detected')

        # Detect mouth
        mouth = mouthCascade.detectMultiScale(roi_img, 1.3, 20)
        if len(mouth) >= 1:
            votes += 1
